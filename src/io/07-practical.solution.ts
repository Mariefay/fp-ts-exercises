...
export const loadConfig = (env: string): IOE.IOEither<string, Config> =>
  () => {
    const config = configStore[env]
    return config ? E.right(config) : E.left(`Config not found for environment: ${env}`)
  }

export const validateConfig = (config: Config): IOE.IOEither<string, Config> =>
  () => {
    if (config.timeout <= 0) {
      return E.left('Invalid timeout: must be greater than 0')
    }
    if (config.retries <= 0) {
      return E.left('Invalid retries: must be greater than 0')
    }
    return E.right(config)
  }

export const loadAndValidateConfig = (env: string): IOE.IOEither<string, Config> =>
  pipe(
    loadConfig(env),
    IOE.flatMap(validateConfig)
  )
