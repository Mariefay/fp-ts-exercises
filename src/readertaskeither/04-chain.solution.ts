...
export const fetchUserWithCompany = (userId: number): RTE.ReaderTaskEither<Env, string, { user: string; company: string }> =>
  pipe(
    fetchUser(userId),
    RTE.flatMap((user) =>
      pipe(
        fetchCompany(user.companyId),
        RTE.map((company) => ({ user: user.name, company: company.name }))
      )
    )
  )
