...
const validateAddress = (address: string): TaskEither<DeliveryError, string> => {
...
const deliverPackage = (pkg: Package): TaskEither<DeliveryError, DeliveryResult> => {
  return () => Promise.resolve(
    Math.random() < 0.1
...
const processDelivery = (pkg: Package): TaskEither<DeliveryError, DeliveryResult> => {
  return pipe(
