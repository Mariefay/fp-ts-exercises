...
export const explorePath = (pathName: string, hasKey: boolean, goldAmount: number): Either<AdventureError, Treasure> => {
  ...
  return right(treasures[pathName] || { name: 'Mysterious Artifact', value: 100 });
};
...
export const formatResult = (result: Either<AdventureError, Treasure>): string => {
  return pipe(
    ...
    fold(
      ...
      (treasure: Treasure) => `Found treasure: ${treasure.name} worth ${treasure.value} gold!`
    )
  );
};
