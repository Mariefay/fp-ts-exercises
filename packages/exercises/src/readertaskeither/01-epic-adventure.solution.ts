import { ReaderTaskEither, ask, right, chain, fromTaskEither } from 'fp-ts/ReaderTaskEither';
...
export const validatePlayerLevel = (quest: Quest): ReaderTaskEither<GameConfig, AdventureError, Quest> => {
  return pipe(
    ask<GameConfig>(),
    chain((config) => fromTaskEither(() => Promise.resolve(
...
export const completeQuest = (quest: Quest): ReaderTaskEither<GameConfig, AdventureError, QuestResult> => {
  return pipe(
    ask<GameConfig>(),
    chain((config) => fromTaskEither(() => {
      const successRate = config.difficulty === 'easy' ? 0.9 : config.difficulty === 'medium' ? 0.7 : 0.5;
...
export const startAdventure = (quest: Quest): ReaderTaskEither<GameConfig, AdventureError, QuestResult> => {
  return pipe(
    validatePlayerLevel(quest),
    chain(completeQuest)
  );
};
