...
const levelLens = {
  ...
  set: (level: number) => (player: Player): Player => ({
    ...player,
    stats: { ...player.stats, level }
  }),
  modify: (f: (level: number) => number) => (player: Player): Player => ({
    ...player,
...
const healthLens = {
  ...
  set: (health: number) => (player: Player): Player => ({
    ...player,
    stats: { ...player.stats, health }
  }),
  modify: (f: (health: number) => number) => (player: Player): Player => ({
    ...player,
...
const goldLens = {
  ...
  set: (gold: number) => (player: Player): Player => ({
    ...player,
    inventory: { ...player.inventory, gold }
  }),
  modify: (f: (gold: number) => number) => (player: Player): Player => ({
    ...player,
...
export const addGold = (amount: number) => (player: Player): Player => {
  return goldLens.modify(gold => gold + amount)(player);
};

export const healPlayer = (amount: number) => (player: Player): Player => {
  return healthLens.modify(health => Math.min(health + amount, 100))(player);
};
